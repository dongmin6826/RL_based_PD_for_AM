...
class GymInterface(gym.Env):
    ...
    def reset(self):
        ...
        self.PD_env, self.PD_tree, self.decomposed_parts = env.create_env()
        return env.cap_current_state(self.PD_env, self.PD_tree, self.decomposed_parts)

    def step(self, action):
        # Update the action of the agent
        if RL_ALGORITHM == "PPO":
            self.PD_env, self.PD_tree, self.decomposed_parts = env.decompose_parts(
                action)
        # Capture the next state of the environment
        next_state = env.cap_current_state(
            self.PD_env, self.PD_tree, self.decomposed_parts)
